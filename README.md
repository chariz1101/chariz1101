<div align="center">

# It's Chariz, by the way! 👋

👩‍💻 **CS Graduate (AI Major)** • **Robotics Enthusiast** • **Full-Stack Developer**

I'm a developer passionate about bridging the gap between hardware and software.

</div>

## 🛠 Tech Stack
**💻 Web & Mobile Development**
- **Frameworks:** Next.js 15.5, React 19, Flutter
- **Styling:** Tailwind CSS 4, Figma (UI/UX)
- **Languages:** TypeScript, JavaScript, Dart, Python

**🤖 Robotics & Embedded Systems**
- **Hardware:** Arduino, ESP32, ET Board
- **Core:** C++ for Embedded, Sensors & Actuators
- **Tools:** ROS, 3D Modeling & Printing

## 🎨 Creative Skills
- 🖌️ UI/UX Design & Prototyping
- 🎨 Graphic Design & Layouting
- 🎥 Video Editing & Videography
- 📷 Photography

## 🌐 Connect with me
- Gmail: [charizdiannefalco@gmail.com](mailto:charizdiannefalco@gmail.com)
- LinkedIn: [Chariz Dianne Falco](https://www.linkedin.com/in/chariz-dianne-falco-12350626a/)

<div align="center">
<i>Let's create amazing things together! 😄🌟</i>
</div>
